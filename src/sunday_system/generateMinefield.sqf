	...
	for "_i" from 1 to (([1,3] call BIS_fnc_randomInt) min (count _minefieldPlaces)) step 1 do {
		...
		
		{
			if (_x inArea _mineMarker) then {
				...
				["Land_Sign_Mines_F", _x, (random 360)] call dro_createSimpleObject;
			};
		} forEach _grid;		
		...
		for "_i" from 1 to (_numSigns) step 1 do {
			...
			if (_i % 3 == 0) then {
				["Land_Sign_WarningUnexplodedAmmo_F", _signPos, ([_signPos, _thisPos] call BIS_fnc_dirTo)] call dro_createSimpleObject;	
			} else {
				["Land_Sign_Mines_F", _signPos, ([_signPos, _thisPos] call BIS_fnc_dirTo)] call dro_createSimpleObject;	
			};								
