...
if (count _travelPositions > 0) then {
	...
	_trgRelease setTriggerStatements [
		...
		", 
		""];
	_trgRelease setVariable ["powChar", _powChar];		
	_trgRelease setVariable ["thisTask", _joinSubTaskName];
	_trgRelease setVariable ["allGuards", _allGuards];	
} else {
	_spawnStationary = true;
};
...
if ((count _firstNames > 0) && (count _lastNames > 0)) then {
	...
	[_powChar, _firstName, _lastName, (speaker _powChar)] remoteExec ["sun_setNameMP", 0, true];
};
if (random 1 > 0.5) then {
