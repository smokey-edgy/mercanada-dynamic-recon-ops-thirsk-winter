...
_vehicleTypes = [];
if (count (((AOLocations select _AOIndex) select 2) select 0) > 1) then {
	_vehicleTypes pushBackUnique "GROUNDROAD";	
};
if (count (((AOLocations select _AOIndex) select 2) select 4) > 1) then {
...
if (_wreckType isKindOf "Helicopter") then {
	...
	diag_log (((AOLocations select _AOIndex) select 2) select 4);
	...
	if ("GROUNDROAD" in _vehicleTypes) then {
		...
		diag_log (((AOLocations select _AOIndex) select 2) select 0);
		_thisPos = [(((AOLocations select _AOIndex) select 2) select 0)] call sun_selectRemove;
	} else {
		diag_log "Wreck type ground, flat pos close";
		diag_log (((AOLocations select _AOIndex) select 2) select 4);
		_thisPos = [(((AOLocations select _AOIndex) select 2) select 4)] call sun_selectRemove;
	};	
};
