		...
		if (count (roadsConnectedTo _thisRoad) <= 2 && _continue) then {
			...
			for "_roadSide" from 0 to 180 step 180 do {	
				...
				if (random 1 > 0.25) then {
					...
					diag_log ((_dir - 90)+_roadSide);
					_spawnPos = [_thisPos, 4, ((_dir - 90)+_roadSide)] call BIS_fnc_relPos;					
					if (random 1 > 0.5) then {
						// Long shelter
						_shelter = selectRandom _marketSheltersLong;
						_shelters pushBack ([(_shelter select 0), _spawnPos, _dir + (_shelter select 1) + _roadSide] call dro_createSimpleObject);
						...
						for "_i" from -_numShelters to _numShelters step 6 do {
							...
							if (random 1 > 0.25) then {
								_shelters pushBack ([(_shelter select 0), _spawnPosNew, _dir + (_shelter select 1) + _roadSide] call dro_createSimpleObject);						
							} else {						
								_shelters pushBack ([(_shelter select 0), _spawnPosNew, _dir + (_shelter select 1) + _roadSide] call dro_createSimpleObject);							 						
							};
					...
					_thisPos = getPos _thisRoad;
					_spawnPos = [_thisPos, 3, (_dir - 90)+_roadSide] call BIS_fnc_relPos;
