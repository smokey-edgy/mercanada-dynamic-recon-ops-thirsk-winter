...
_radioTaskIntel = "";	
_thisTask = [];
diag_log format ["taskIntel = %1", taskIntel];
if (count taskIntel > 0) then {
	for "_i" from 1 to ((round((count taskIntel)*0.4)) max 1) step 1 do {		
		...
		diag_log format ["_thisTask = %1", _thisTask];
		if ([_thisTask select 0] call BIS_fnc_taskExists) then {
			...
			diag_log format ["_taskDescData = %1", _taskDescData];
			diag_log format ["_taskData = %1", _taskData];
			...
			diag_log format ["_taskDesc = %1", _taskDesc];		
			switch (_intelType) do {
				case "MARKER": {
					...
					diag_log format ["Moving task destination: %1, %2", _taskTitle, _marker];
	...
	if (_chanceUnitReveal > 0.4) then {
		...
		for "_j" from 0 to ([1,3] call BIS_fnc_randomInt) step 1 do {
			...
			if (side _unit == enemySide) then {
				[(grpNetId call BIS_fnc_groupFromNetId), [_unit, 3.5]] remoteExec ["reveal", 0];					
			};
		};
		_unitRevealUsed = true;
		*/
...
if (!isNull _caller) then {
	...
	[_caller, _firstSentence, _radioDesc, _radioTaskIntel] spawn {
		...
		[(_this select 0), (_this select 1)] remoteExec ["groupChat", 0];
		sleep 3;
		[(_this select 0), (_this select 2)] remoteExec ["groupChat", 0];
		sleep 6;
		if (count (_this select 3) > 0) then {
			[(_this select 0), (_this select 3)] remoteExec ["groupChat", 0];
		};
	};
};
