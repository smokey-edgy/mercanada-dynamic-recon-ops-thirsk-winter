...
{	
	...
	_deleteControl ctrlSetPosition [0, ((_forEachIndex) * _lineSpacing), 0.05, 0.05];	
	...
	_nameControl ctrlSetPosition [0.05, ((_forEachIndex) * _lineSpacing), 0.2, 0.05];		
	if (isPlayer _x) then {		
	...
	if ((player == _x) OR ((player == _dialogPlayer) && (!isPlayer _x))) then {
		...
		_loadoutControl ctrlSetPosition [0.25, ((_forEachIndex) * _lineSpacing), 0.3, 0.05];	
	...
	_VAControl ctrlSetPosition [0.55, ((_forEachIndex) * _lineSpacing), 0.05, 0.05];	
...
if (player == _dialogPlayer) then {
	_validVehicles = [];
	{
		...
		if (({_x != "CMFlareLauncher"} count ((configfile >> "CfgVehicles" >> _x >> "weapons") call BIS_fnc_getCfgData)) == 0) then {
