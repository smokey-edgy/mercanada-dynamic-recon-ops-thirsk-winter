...
switch (_hvtStyle) do {
	...
	case "OUTSIDETRAVEL": {
		_possibleLocTypes = [];
		{
			if (count ((_x select 2) select 0) > 0) then {_possibleLocTypes pushBack 0};
			if (count ((_x select 2) select 2) > 0) then {_possibleLocTypes pushBack 2};
			if (count ((_x select 2) select 3) > 0) then {_possibleLocTypes pushBack 3};
			if (count ((_x select 2) select 4) > 0) then {_possibleLocTypes pushBack 4};
		} forEach AOLocations;
...
if ((count _firstNames > 0) && (count _lastNames > 0)) then {		
	[_hvtChar, (selectRandom _firstNames), (selectRandom _lastNames), (speaker _hvtChar)] remoteExec ["sun_setNameMP", 0, true];
};
if (random 1 > 0.5) then {
