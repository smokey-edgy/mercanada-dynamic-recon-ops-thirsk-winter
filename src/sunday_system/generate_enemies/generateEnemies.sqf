...
if (_AOIndex == 0) then {
	...
	[(8*_sizeMod)] call dro_localBuildingPatrol;
	{
		_chance = (random 1);
		if (_chance > 0.3) then {
...
for "_g" from 1 to (round(([5, 8] call BIS_fnc_randomInt) * _sizeMod) min (count (((AOLocations select _AOIndex) select 2) select 7))) step 1 do {	
	[(selectRandom (((AOLocations select _AOIndex) select 2) select 7))] call dro_spawnEnemyGarrison;
};

// Infantry patrols
_numInf = round ((([2,4] call BIS_fnc_randomInt) * aiMultiplier) * _sizeMod);	
